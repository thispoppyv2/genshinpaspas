...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    const cellSize = 512;
    const padding = 4; // Add padding to prevent texture bleeding
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  const isExternalLink = activeItem?.link?.startsWith("http");
  ...
  const buttonElement = (
    <div
      ...
      `}
    >
      <div className="gap-2 select-none relative text-[#060010] top-[2px]  items-center flex justify-center">
    ...
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        <>
          ...
          ${
            isMoving
              ? "opacity-0 scale-0 pointer-events-none duration-[100ms]"
              : "opacity-100 scale-100 pointer-events-auto duration-[500ms]"
          }
        `}
          >
            {activeItem.title}
        ...
        `}
          ...
          {isExternalLink ? (
            ...
            <Link href={activeItem.link || ""}>{buttonElement}</Link>
          )}
        </>
      )}
    </div>
